...
type StorageEventType = 'issue_created' | 'issue_updated' | 'bid_created' | 'bid_updated' | 'donation_created' | 'ngo_request_created' | 'ngo_request_updated';
...
export const addNGORequest = (request: NGORequest): void => {
  ...
  emitEvent('ngo_request_created', request);
};
...
export const updateNGORequest = (requestId: string, updates: Partial<NGORequest>): void => {
  ...
  if (index !== -1) {
    ...
    saveNGORequests(requests);
    emitEvent('ngo_request_updated', requests[index]);
  }
};
...
export const getAllNGORequests = (): NGORequest[] => {
  return getNGORequests();
};

export const updateNGORequestStatus = (
  requestId: string, 
